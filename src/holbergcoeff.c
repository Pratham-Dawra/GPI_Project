...
float *holbergcoeff(GlobVar * gv)
{
    float hcall[5][6][7] = {
        {
         ...
         { 4.0, 160083.0 / 131072.0, -12705.0 / 131072.0, 22869.0 / 1310720.0, -5445.0 / 1835008.0, 847.0 / 2359296.0,
          -63.0 / 2883584.0}
          },
        {
    ...
    if ((gv->FDORDER != 2) && (gv->FDORDER != 4) && (gv->FDORDER != 6) && (gv->FDORDER != 8) && (gv->FDORDER != 10) &&
        (gv->FDORDER != 12)) {
        log_fatal("Error in selection of FD coefficients: wrong FDORDER!\n");
    }
    if ((gv->MAXRELERROR < 0) || (gv->MAXRELERROR > 4)) {
        log_fatal("Error in selection of FD coefficients: wrong choice of maximum relative error!\n");
    }

    int nl = (gv->FDORDER / 2) - 1;
    float *hc = vector(0, 6);
