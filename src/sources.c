...
float **sources(int *nsrc, GlobVar *gv)
{
  ...
  if (gv->MPID==0){
      ...
      while(fgets(buffer, STRING_SIZE, fpsrc))
	{
      ...
      if ((nsrc)==0) log_warn("Could not determine number of sources parameter sets; assuming %d.\n",(*nsrc=0));
      else log_info("Number of source positions: %d\n",*nsrc);
	...
	*/
	...
	while (fgets(cline,255,fpsrc)) {
	  ...
	  if ((strchr(cline,'#')==0) && (sscanf(cline,"%s",bufferstring)==1)) {
	    ++l;
	    if (l>*nsrc) log_fatal("sources.c: buffer not large enough to store all sources - programming error.\n");
	...
	*/
	...
	while (fgets(cline,255,fpsrc)) {
	  ...
	  if ((strchr(cline,'#')==0) && (sscanf(cline,"%s",bufferstring)==1)) {
	    ++l;
	    if (l>*nsrc) log_fatal("sources.c: buffer not large enough to store all sources - programming error.\n");
	    nvarin=sscanf(cline,"%f%f%f%f%f%f%f%f%f%f",&xsrc, &ysrc, &tshift, &srcpos[5][l], &srcpos[6][l], &srcpos[9][l], 
	...
	*/
	...
	while (fgets(cline,255,fpsrc)) {
	  ...
	  if ((strchr(cline,'#')==0) && (sscanf(cline,"%s",bufferstring)==1)) {
	    ++l;
	    if (l>*nsrc) log_fatal("sources.c: buffer not large enough to store all sources - programming error.\n");
	    nvarin=sscanf(cline,"%f%f%f%f%f%f%f%f%f%f",&xsrc, &ysrc, &tshift, &srcpos[9][l], &srcpos[10][l], &srcpos[6][l], 
	...
	if (gv->SOURCE_SHAPE > 4) {
	  log_fatal("Plane wave is only implemented for Ricker, Fuchs-Mueller, sin^3 or an external wavelet! Change parameter SOURCE_SHAPE!\n");
	}
	log_info("------------------------- Source parameters (II) ------------\n");
	...
	else log_fatal("Maximum depth of plane wave exceeds model depth.\n");
      }
  ...
  if (gv->MPID==0){
    ...
    if (4 >= gv->SOURCE_SHAPE) {
      ...
      log_info("  Shot         x         y   tshift     fmin     fmax       fc     amp   tsweep    twave azimuth type\n");
    }
	...
	log_info("%6d %9.2f %9.2f %8.4f %8.2f %8.2f %7.2f %4d\n",
		 l,srcpos[1][l],srcpos[2][l],srcpos[4][l],srcpos[5][l],srcpos[6][l],srcpos[7][l],(int)srcpos[8][l]);
      } else if (5 == gv->SOURCE_SHAPE) {
	log_info("%6d %9.2f %9.2f %8.4f %8.2f %8.2f %4.1f %7.2f %7.2f %7.2f %4d\n",
		 l,srcpos[1][l],srcpos[2][l],srcpos[4][l],srcpos[5][l],srcpos[6][l],srcpos[9][l],srcpos[10][l],srcpos[11][l],srcpos[7][l],(int)srcpos[8][l]);
      } else if (6 == gv->SOURCE_SHAPE) {
