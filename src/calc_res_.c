              ...
              AcqVar *acq, GlobVar *gv)
{
    ...
    switch (gv->ADJOINT_TYPE) {
      case 1:                  /* vx only */
          ...
          readseis(ishot, section_obs->vx, acq, ntr_loc, gv->NS, 1, gv);
          if (gv->FILT == 1) {
              filt_seis(section_obs->vx, ntr_loc, gv->NS, finv[nf - 1], gv);
          }
          if (gv->STFI_CALC) {
              if (gv->USE_TW) {
                  time_window(section->vx, section_obs->vx, acq->recpos_loc, gv->NTRG, ntr_loc, ishot, groupnum, 1, gv);
              }
              /* STFI - Source time function inversion */
              stfi_calc(ishot, section, section_obs, signals, ntr_loc, 1024, gv);
          }
          /* calculate residuals and L2 Norm
          if (res_switch) {
              if (gv->USE_TW) {
                  time_window(section->vx, section_obs->vx, acq->recpos_loc, gv->NTRG, ntr_loc, ishot, groupnum, 0, gv);
              }
      ...
      case 2:                  /* vy only */
          ...
          readseis(ishot, section_obs->vy, acq, ntr_loc, gv->NS, 2, gv);
          if (gv->FILT == 1) {
              filt_seis(section_obs->vy, ntr_loc, gv->NS, finv[nf - 1], gv);
          }
          if (gv->STFI_CALC) {
              if (gv->USE_TW) {
                  time_window(section->vy, section_obs->vy, acq->recpos_loc, gv->NTRG, ntr_loc, ishot, groupnum, 1, gv);
              }
              /* STFI - Source time function inversion */
              stfi_calc(ishot, section, section_obs, signals, ntr_loc, 1024, gv);
          }
          ...
          if (res_switch) {
              if (gv->USE_TW) {
                  time_window(section->vy, section_obs->vy, acq->recpos_loc, gv->NTRG, ntr_loc, ishot, groupnum, 0, gv);
              }
      ...
      case 3:                  /* vz only */
          ...
          readseis(ishot, section_obs->vz, acq, ntr_loc, gv->NS, 3, gv);
          if (gv->FILT == 1) {
              filt_seis(section_obs->vz, ntr_loc, gv->NS, finv[nf - 1], gv);
          }
          if (gv->STFI_CALC) {
              if (gv->USE_TW) {
                  time_window(section->vz, section_obs->vz, acq->recpos_loc, gv->NTRG, ntr_loc, ishot, groupnum, 1, gv);
              }
              /* STFI - Source time function inversion */
              stfi_calc(ishot, section, section_obs, signals, ntr_loc, 1024, gv);
          }
          ...
          if (res_switch) {
              if (gv->USE_TW) {
                  time_window(section->vz, section_obs->vz, acq->recpos_loc, gv->NTRG, ntr_loc, ishot, groupnum, 0, gv);
              }
      ...
      case 5:                  /* pressure only */
          ...
          readseis(ishot, section_obs->p, acq, ntr_loc, gv->NS, 4, gv);
          if (gv->FILT == 1) {
          ...
          if (gv->STFI_CALC) {
              /* STFI - Source time function inversion */
              if (gv->USE_TW) {
                 time_window(section->p, section_obs->p, acq->recpos_loc, gv->NTRG, ntr_loc, ishot, groupnum, 1, gv);
              }
              stfi_calc(ishot, section, section_obs, signals, ntr_loc, 1024, gv);
          }
          /* calculate residuals and L2 Norm
          if (res_switch) {
              if (gv->USE_TW) {
                  time_window(section->p, section_obs->p, acq->recpos_loc, gv->NTRG, ntr_loc, ishot, groupnum, 0, gv);
              }
