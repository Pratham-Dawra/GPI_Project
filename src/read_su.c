...
off_t su_filesize(FILE *filep)
{
  ...
  if (ierr != 0) log_fatal("Could not seek to end of SU file.\n");
  return ftell(filep);
}
...
size_t su_get_nt(FILE *filep, unsigned short *ns, unsigned short *dt, short *delrt)
{
  ...
  *delrt = 0;
  ...
  if (ierr != 0) log_fatal("Could not seek to beginning of SU file.\n");
  
  ierr = fread(&header, sizeof(SUhead), 1, filep);
  if (ierr != 1) log_fatal("Short read in SU file.\n");

  if (header.ns < 1) log_fatal("SU file has no number of samples (ns) in first SU header.\n");
...
int su_read_trace(FILE *filep, size_t n, unsigned short ns, bool b_seek, SUhead *header, float *data)
{
  ...
  if (b_seek) {
    ...
    if (ierr != 0) log_fatal("Could not seek to offset %ld of SU file.\n", fileoffset);
  }

  // read SU trace header if requested
  if (header) {
    ierr = fread(header, sizeof(SUhead), 1, filep);
    if (ierr != 1) log_fatal("Short read in SU file while reading header.\n");
  } else {
    ierr = fseek(filep, sizeof(SUhead), SEEK_CUR);
    if (ierr != 0) log_fatal("Could not skip over trace header in SU file.\n");
  }

  // read SU trace data
  ierr = fread(data, sizeof(float), ns, filep);
  if (ierr != ns) log_fatal("Short read in SU file while reading data.\n");
...
int main(int argc, char **argv) 
{
  ...
  if (!filep) log_fatal("Could not open SU file %s for reading.\n", sufile);

  SUgather gather;
  size_t nt_read = su_read_file(filep, &gather);

  if (nt_read != gather.nt) log_error("Internal 'number of traces' mismatch. Programming error.\n");
