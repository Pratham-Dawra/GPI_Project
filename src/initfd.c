...
void op_ac_s_fd2(int i, int j, float *vxx, float *vyy, MemWavefield * mpw)
{
    *vxx = hc[1] * (mpw->pvx[j][i] - mpw->pvx[j][i - 1]) * dhi;
    *vyy = hc[1] * (mpw->pvy[j][i] - mpw->pvy[j - 1][i]) * dhi;
}

void op_ac_s_fd4(int i, int j, float *vxx, float *vyy, MemWavefield * mpw)
{
    *vxx = (hc[1] * (mpw->pvx[j][i] - mpw->pvx[j][i - 1]) + hc[2] * (mpw->pvx[j][i + 1] - mpw->pvx[j][i - 2])) * dhi;
    *vyy = (hc[1] * (mpw->pvy[j][i] - mpw->pvy[j - 1][i]) + hc[2] * (mpw->pvy[j + 1][i] - mpw->pvy[j - 2][i])) * dhi;
}

void op_ac_s_fd6(int i, int j, float *vxx, float *vyy, MemWavefield * mpw)
{
    *vxx =
        (hc[1] * (mpw->pvx[j][i] - mpw->pvx[j][i - 1]) + hc[2] * (mpw->pvx[j][i + 1] - mpw->pvx[j][i - 2]) +
         hc[3] * (mpw->pvx[j][i + 2] - mpw->pvx[j][i - 3])) * dhi;
...
void op_ac_s_fd8(int i, int j, float *vxx,  float *vyy, MemWavefield * mpw)
{
    *vxx =
        (hc[1] * (mpw->pvx[j][i] - mpw->pvx[j][i - 1]) + hc[2] * (mpw->pvx[j][i + 1] - mpw->pvx[j][i - 2]) +
         hc[3] * (mpw->pvx[j][i + 2] - mpw->pvx[j][i - 3]) + hc[4] * (mpw->pvx[j][i + 3] - mpw->pvx[j][i - 4])) * dhi;
...
void op_ac_s_fd10(int i, int j, float *vxx, float *vyy, MemWavefield * mpw)
{
    *vxx =
        (hc[1] * (mpw->pvx[j][i] - mpw->pvx[j][i - 1]) + hc[2] * (mpw->pvx[j][i + 1] - mpw->pvx[j][i - 2]) +
         hc[3] * (mpw->pvx[j][i + 2] - mpw->pvx[j][i - 3]) + hc[4] * (mpw->pvx[j][i + 3] - mpw->pvx[j][i - 4]) +
...
void op_ac_s_fd12(int i, int j, float *vxx, float *vyy, MemWavefield * mpw)
{
    *vxx =
        (hc[1] * (mpw->pvx[j][i] - mpw->pvx[j][i - 1]) + hc[2] * (mpw->pvx[j][i + 1] - mpw->pvx[j][i - 2]) +
         hc[3] * (mpw->pvx[j][i + 2] - mpw->pvx[j][i - 3]) + hc[4] * (mpw->pvx[j][i + 3] - mpw->pvx[j][i - 4]) +
...
void op_ac_v_fd2(int i, int j, float *sxx_x,  float *syy_y, MemWavefield * mpw)
{
    *sxx_x = hc[1] * (mpw->psxx[j][i + 1] - mpw->psxx[j][i]);
    *syy_y = hc[1] * (mpw->psyy[j + 1][i] - mpw->psyy[j][i]);
}

void op_ac_v_fd4(int i, int j, float *sxx_x, float *syy_y, MemWavefield * mpw)
{
    *sxx_x = hc[1] * (mpw->psxx[j][i + 1] - mpw->psxx[j][i]) + hc[2] * (mpw->psxx[j][i + 2] - mpw->psxx[j][i - 1]);
    *syy_y = hc[1] * (mpw->psyy[j + 1][i] - mpw->psyy[j][i]) + hc[2] * (mpw->psyy[j + 2][i] - mpw->psyy[j - 1][i]);
}

void op_ac_v_fd6(int i, int j, float *sxx_x, float *syy_y, MemWavefield * mpw)
{
    *sxx_x =
        hc[1] * (mpw->psxx[j][i + 1] - mpw->psxx[j][i]) + hc[2] * (mpw->psxx[j][i + 2] - mpw->psxx[j][i - 1]) +
        hc[3] * (mpw->psxx[j][i + 3] - mpw->psxx[j][i - 2]);
...
void op_ac_v_fd8(int i, int j, float *sxx_x,  float *syy_y, MemWavefield * mpw)
{
    *sxx_x =
        hc[1] * (mpw->psxx[j][i + 1] - mpw->psxx[j][i]) + hc[2] * (mpw->psxx[j][i + 2] - mpw->psxx[j][i - 1]) +
        hc[3] * (mpw->psxx[j][i + 3] - mpw->psxx[j][i - 2]) + hc[4] * (mpw->psxx[j][i + 4] - mpw->psxx[j][i - 3]);
...
void op_ac_v_fd10(int i, int j, float *sxx_x,  float *syy_y, MemWavefield * mpw)
{
    *sxx_x =
        hc[1] * (mpw->psxx[j][i + 1] - mpw->psxx[j][i]) + hc[2] * (mpw->psxx[j][i + 2] - mpw->psxx[j][i - 1]) +
        hc[3] * (mpw->psxx[j][i + 3] - mpw->psxx[j][i - 2]) + hc[4] * (mpw->psxx[j][i + 4] - mpw->psxx[j][i - 3]) +
...
void op_ac_v_fd12(int i, int j, float *sxx_x, float *syy_y, MemWavefield * mpw)
{
    *sxx_x =
        hc[1] * (mpw->psxx[j][i + 1] - mpw->psxx[j][i]) + hc[2] * (mpw->psxx[j][i + 2] - mpw->psxx[j][i - 1]) +
        hc[3] * (mpw->psxx[j][i + 3] - mpw->psxx[j][i - 2]) + hc[4] * (mpw->psxx[j][i + 4] - mpw->psxx[j][i - 3]) +
...
static void update_fd_fct_ptr(int order, GlobVar * gv)
{
    ...
    }else
    {
        switch (order) {
            case 2:
                gv->FDOP_AC_S = &op_ac_s_fd2;
                gv->FDOP_AC_V = &op_ac_v_fd2;
                break;
            case 4:
                gv->FDOP_AC_S = &op_ac_s_fd4;
                gv->FDOP_AC_V = &op_ac_v_fd4;
                break;
            case 6:
                gv->FDOP_AC_S = &op_ac_s_fd6;
                gv->FDOP_AC_V = &op_ac_v_fd6;
                break;
            case 8:
                gv->FDOP_AC_S = &op_ac_s_fd8;
                gv->FDOP_AC_V = &op_ac_v_fd8;
                break;
            case 10:
                gv->FDOP_AC_S = &op_ac_s_fd10;
                gv->FDOP_AC_V = &op_ac_v_fd10;
                break;
            case 12:
                gv->FDOP_AC_S = &op_ac_s_fd12;
                gv->FDOP_AC_V = &op_ac_v_fd12;
                break;
            default:
                log_fatal("Unsupported FDORDER encountered.\n");
                break;
