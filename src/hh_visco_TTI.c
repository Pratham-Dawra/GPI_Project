...
void model_visco_tti(float  **  rho, float **  pc11, float **  pc33, float **  pc13, float **  pc55, float **  pc15, float **  pc35,
	float **  ptau11, float **  ptau33, float **  ptau13, float **  ptau55, float ** ptau15, float ** ptau35, float *  eta){
...
/* anisotropic case, values from
      ...
      const float TAU11=TAU, TAU33=TAU, TAU55=1.0*TAU;
   ...
   if (MYID==0){
	...
	if (WRITE_MODELFILES==1) {
        ...
        sprintf(modfile,"%s.SOFI2D.tau15",MFILE);
        writemod(modfile,ptau15,3);
        MPI_Barrier(MPI_COMM_WORLD);
        if (MYID==0) mergemod(modfile,3);
 
        sprintf(modfile,"%s.SOFI2D.tau35",MFILE);
        writemod(modfile,ptau35,3);
        MPI_Barrier(MPI_COMM_WORLD);
        if (MYID==0) mergemod(modfile,3);
