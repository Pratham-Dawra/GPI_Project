...
void exchange_v(int nd, float ** vx, float ** vy,
	...
	/*
 	MPI_Bsend(&bufferlef_to_rig[1][1],(gv->NY)*fdo2,MPI_FLOAT,gv->INDEX[1],gv->TAG1,MPI_COMM_WORLD);
	MPI_Barrier(MPI_COMM_WORLD);
	MPI_Recv(&bufferlef_to_rig[1][1],(gv->NY)*fdo2,MPI_FLOAT,gv->INDEX[2],gv->TAG1,MPI_COMM_WORLD,&status);
	MPI_Bsend(&bufferrig_to_lef[1][1],(gv->NY)*fdo2,MPI_FLOAT,gv->INDEX[2],gv->TAG2,MPI_COMM_WORLD);
	MPI_Barrier(MPI_COMM_WORLD);
	MPI_Recv(&bufferrig_to_lef[1][1],(gv->NY)*fdo2,MPI_FLOAT,gv->INDEX[1],gv->TAG2,MPI_COMM_WORLD,&status);
	 */
