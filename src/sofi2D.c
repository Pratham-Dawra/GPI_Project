    ...
    ** psi_vxx=NULL, ** psi_vyy=NULL, ** psi_vxy=NULL, ** psi_vyx=NULL, ** psi_vxxs=NULL;
    ...
    switch (gv.LOG) {
        ...
        case 2:
            ...
            if ((gv.FP=fopen(gv.LOG_FILE, "w")) == NULL) {
                declare_error ("Opening log file failed.");
            }
            if (MYID == 0) {
                note(stdout, &gv);
            } else {
                fprintf(gv.FP, "This is the log file %s generated by PE %d \n\n", gv.LOG_FILE, MYID);
            }
            break;
        case 3:
    ...
    gv.NT = iround(gv.TIME/gv.DT);      /* number of timesteps */
    ...
    time2 = MPI_Wtime();
    fprintf ( gv.FP, "\n\n\n **************************************************\n" );
    fprintf ( gv.FP, " *********** STARTING TIME STEPPING ***************\n" );
    fprintf ( gv.FP, " **************************************************\n\n" );
    if ( MYID == 0 ) {
        fprintf ( gv.FP, " real time before starting time loop: %4.2f s.\n",
                 time2 - time1 );
    }
    ...
    for ( ishot = 1; ishot <= nshots; ishot++ ) {
        ...
        if ( gv.RUN_MULTIPLE_SHOTS ) {
            ...
            fprintf (
                     gv.FP,
                     "==================================================================================\n\n" );
            fprintf ( gv.FP, " Parameter for shot %d are:\n", ishot );
            fprintf (
                     gv.FP,
        ...
        if (0==MYID && 1==gv.SIGOUT) {
            ...
            outseis_glob(gv.FP,fopen(sigf,"w"),signals,recpos,recpos_loc,1,srcpos_loc,nsrc,gv.NT,gv.SIGOUT_FORMAT,ishot,0, &gv);
        }
		...
		debug_check_matrix(pvy, nt, gv.NX, gv.NY, 121, 0, "pvy");
            ...
            if ( ( MYID == 0 )
                ...
                fprintf ( gv.FP, " particle velocity exchange between PEs ..." );
            }
            ...
            if ( ( MYID == 0 )
                ...
                fprintf ( gv.FP, " stress exchange between PEs ..." );
            }
            /*---------------------------------------------------------------*/
            ...
            if ( ( gv.SNAP ) && ( nt == lsnap ) && ( nt <= gv.TSNAP2 / gv.DT ) ) {
                ...
                snap ( gv.FP, nt, ++nsnap, pvx, pvy, psxx, psyy, pu, ppi, hc, &gv );
            ...
            if ( ( MYID == 0 )	&& ( ( nt + ( gv.OUTNTIMESTEPINFO - 1 ) ) % gv.OUTNTIMESTEPINFO ) == 0 ) {
                ...
                fprintf ( gv.FP, " total real time for timestep %d : %4.3f s.\n", nt, time8 - time3 );
            }
        }
        /*---------------------------------------------------------------*/
        ...
        if ( gv.SEISMO ) {
            ...
            switch ( gv.SEISMO ) {
                case 1 : /* particle velocities only */
                    ...
                    if ( MYID==0 ) saveseis_glob ( gv.FP,seismo_fulldata,recpos,recpos_loc,ntr_glob,srcpos,ishot,ns,1, &gv );
                    catseis ( sectionvy, seismo_fulldata, recswitch, ntr_glob,ns );
                    if ( MYID==0 ) saveseis_glob ( gv.FP,seismo_fulldata,recpos,recpos_loc,ntr_glob,srcpos,ishot,ns,2, &gv );
                    
                    break;
                case 2 : /* pressure only */
                    catseis ( sectionp, seismo_fulldata, recswitch, ntr_glob,ns );
                    if ( MYID==0 ) saveseis_glob ( gv.FP,seismo_fulldata,recpos,recpos_loc,ntr_glob,srcpos,ishot,ns,4, &gv );
                    
                    break;
                case 3 : /* curl and div only */
                    catseis ( sectiondiv, seismo_fulldata, recswitch, ntr_glob,ns );
                    if ( MYID==0 ) saveseis_glob ( gv.FP,seismo_fulldata,recpos,recpos_loc,ntr_glob,srcpos,ishot,ns,5, &gv );
                    catseis ( sectioncurl, seismo_fulldata, recswitch, ntr_glob,ns );
                    if ( MYID==0 ) saveseis_glob ( gv.FP,seismo_fulldata,recpos,recpos_loc,ntr_glob,srcpos,ishot,ns,6, &gv );
...
/*    if ( WEQ==7 ) {
    ...
    if (MYID == 0) {
        ...
        fprintf(gv.FP, " Average times for \n");
        fprintf(gv.FP, "   velocity update:  \t %5.6f seconds  \n", time_av_v_update);
        fprintf(gv.FP, "   stress update:  \t %5.6f seconds  \n", time_av_s_update);
        fprintf(gv.FP, "   velocity exchange:  \t %5.6f seconds  \n", time_av_v_exchange);
        fprintf(gv.FP, "   stress exchange:  \t %5.6f seconds  \n", time_av_s_exchange);
        fprintf(gv.FP, "   timestep:  \t\t %5.6f seconds  \n\n", time_av_timestep);
        cpu_time = clock()-cpu_time1;
        fprintf(gv.FP, " CPU time of program per PE: %li seconds.\n", cpu_time/CLOCKS_PER_SEC);
        time8 = MPI_Wtime();
        fprintf(gv.FP, " Total real time of program: %4.3f seconds.\n\n", time8-time1);
        fprintf(gv.FP," ******************************************************\n" );
        fprintf(gv.FP," **************** SOFI2D has finished *****************\n" );
        fprintf(gv.FP," ******************************************************\n\n" );
    }
