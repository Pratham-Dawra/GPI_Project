...
void read_par_json(const char *fileinp, GlobVar *gv) 
{
  ...
  if (get_string_from_objectlist("LOG_VERBOSITY",number_readobjects,(gv->LOG_VERBOSITY),varname_list, value_list, used_list)) {
  ...
  if (get_int_from_objectlist("FDORDER_TIME",number_readobjects,&(gv->FDORDER_TIME),varname_list, value_list, used_list)) {
    (gv->FDORDER_TIME)=2;
  } else {
  ...
  if (get_int_from_objectlist("MAXRELERROR",number_readobjects,&(gv->MAXRELERROR),varname_list, value_list, used_list))
  ...
  if (get_int_from_objectlist("SOURCE_TYPE",number_readobjects,&(gv->SOURCE_TYPE),varname_list, value_list, used_list))
  ...
  if (get_int_from_objectlist("SIGOUT",number_readobjects,&(gv->SIGOUT),varname_list, value_list, used_list)) {
    log_info("For your information: variables SIGOUT, SIGOUT_FILE & SIGOUT_FORMAT available to output source signal to file.\n");
  } else {
    if ((gv->SIGOUT)==1) {
      if (get_string_from_objectlist("SIGOUT_FILE",number_readobjects,gv->SIGOUT_FILE,varname_list, value_list, used_list)) {
          log_fatal("Variable SIGOUT_FILE could not be retrieved from the json input file!");
      }
      if (get_int_from_objectlist("SIGOUT_FORMAT",number_readobjects,&(gv->SIGOUT_FORMAT),varname_list, value_list, used_list)) {
        log_fatal("Variable SIGOUT_FORMAT could not be retrieved from the json input file!");
      }
    }
  }
  if (get_int_from_objectlist("SOURCE_SHAPE",number_readobjects,&(gv->SOURCE_SHAPE),varname_list, value_list, used_list))
    log_fatal("Variable SOURCE_SHAPE could not be retrieved from the json input file!");
  else {
    if ((gv->SOURCE_SHAPE)==3) {
      if (get_string_from_objectlist("SIGNAL_FILE",number_readobjects,(gv->SIGNAL_FILE),varname_list, value_list, used_list))
  ...
  if (get_int_from_objectlist("SRCREC",number_readobjects,&(gv->SRCREC),varname_list, value_list, used_list))
    ...
    if ((gv->SRCREC)==1) {
      if (get_string_from_objectlist("SOURCE_FILE",number_readobjects,(gv->SOURCE_FILE),varname_list, value_list, used_list))
	log_fatal("Variable SOURCE_FILE could not be retrieved from the json input file!");
      if (get_int_from_objectlist("RUN_MULTIPLE_SHOTS",number_readobjects,&(gv->RUN_MULTIPLE_SHOTS),varname_list, value_list, used_list))
	log_fatal("Variable RUN_MULTIPLE_SHOTS could not be retrieved from the json input file!");
    }
    if ((gv->SRCREC)==2) {
      if (get_float_from_objectlist("PLANE_WAVE_DEPTH",number_readobjects,&(gv->PLANE_WAVE_DEPTH),varname_list, value_list, used_list))
	log_fatal("Variable PLANE_WAVE_DEPTH could not be retrieved from the json input file!");
      else {
	if ((gv->PLANE_WAVE_DEPTH)>0.0) {
	  if (get_float_from_objectlist("PLANE_WAVE_ANGLE",number_readobjects,&(gv->PLANE_WAVE_ANGLE),varname_list, value_list, used_list))
	    log_fatal("Variable PLANE_WAVE_ANGLE could not be retrieved from the json input file!");
	  if (get_float_from_objectlist("TS",number_readobjects,&(gv->TS),varname_list, value_list, used_list))
  ...
  if ((gv->ABS_TYPE)==1) {
    ...
    if (get_float_from_objectlist("K_MAX_CPML",number_readobjects,&(gv->K_MAX_CPML),varname_list, value_list, used_list)) 
  ...
  if ((gv->ABS_TYPE)==2) {
    if (get_float_from_objectlist("DAMPING",number_readobjects,&(gv->DAMPING),varname_list, value_list, used_list))
  ...
  if (get_int_from_objectlist("SNAP",number_readobjects,&(gv->SNAP),varname_list, value_list, used_list))
    ...
    if ((gv->SNAP)>0) {
      if (get_int_from_objectlist("SNAP_FORMAT",number_readobjects,&(gv->SNAP_FORMAT),varname_list, value_list, used_list))
	log_fatal("Variable SNAP_FORMAT could not be retrieved from the json input file!");
      if (get_float_from_objectlist("TSNAP1",number_readobjects,&(gv->TSNAP1),varname_list, value_list, used_list))
	log_fatal("Variable TSNAP1 could not be retrieved from the json input file!");
      if (get_float_from_objectlist("TSNAP2",number_readobjects,&(gv->TSNAP2),varname_list, value_list, used_list))
	log_fatal("Variable TSNAP2 could not be retrieved from the json input file!");
      if (get_float_from_objectlist("TSNAPINC",number_readobjects,&(gv->TSNAPINC),varname_list, value_list, used_list))
	log_fatal("Variable TSNAPINC could not be retrieved from the json input file!");
      if (get_string_from_objectlist("SNAP_FILE",number_readobjects,(gv->SNAP_FILE),varname_list, value_list, used_list))
  ...
  if (get_int_from_objectlist("SEISMO",number_readobjects,&(gv->SEISMO),varname_list, value_list, used_list))
    ...
    if ((gv->SEISMO)>0){
      if (get_string_from_objectlist("REC_FILE",number_readobjects,(gv->REC_FILE),varname_list, value_list, used_list))
	log_fatal("Variable REC_FILE could not be retrieved from the json input file!");
      if (get_string_from_objectlist("SEIS_FILE",number_readobjects,(gv->SEIS_FILE),varname_list, value_list, used_list))
	log_fatal("Variable SEIS_FILE could not be retrieved from the json input file!");
      if (get_int_from_objectlist("READREC",number_readobjects,&(gv->READREC),varname_list, value_list, used_list))
	log_fatal("Variable READREC could not be retrieved from the json input file!");
      else {
	if ((gv->READREC)==0) {
	  if (get_float_from_objectlist("XREC1",number_readobjects,&(gv->XREC1),varname_list, value_list, used_list))
	    log_fatal("Variable XREC1 could not be retrieved from the json input file!");
	  if (get_float_from_objectlist("XREC2",number_readobjects,&(gv->XREC2),varname_list, value_list, used_list))
	    log_fatal("Variable XREC2T could not be retrieved from the json input file!");
	  if (get_float_from_objectlist("YREC1",number_readobjects,&(gv->YREC1),varname_list, value_list, used_list))
	    log_fatal("Variable YREC1 could not be retrieved from the json input file!");
	  if (get_float_from_objectlist("YREC2",number_readobjects,&(gv->YREC2),varname_list, value_list, used_list))
	...
	log_fatal("Variable NDT could not be retrieved from the json input file!");
      if (get_int_from_objectlist("SEIS_FORMAT",number_readobjects,&(gv->SEIS_FORMAT),varname_list, value_list, used_list))
	log_fatal("Variable SEIS_FORMAT could not be retrieved from the json input file!");
      
      if (get_int_from_objectlist("REC_ARRAY",number_readobjects,&(gv->REC_ARRAY),varname_list, value_list, used_list))
	log_fatal("Variable REC_ARRAY could not be retrieved from the json input file!");
      else {
	if ((gv->REC_ARRAY)>0) {
	  if (get_int_from_objectlist("DRX",number_readobjects,&(gv->DRX),varname_list, value_list, used_list))
	    log_fatal("Variable DRX could not be retrieved from the json input file!");
	  if (get_float_from_objectlist("REC_ARRAY_DEPTH",number_readobjects,&(gv->REC_ARRAY_DEPTH),varname_list, value_list, used_list))
	    log_fatal("Variable REC_ARRAY_DEPTH could not be retrieved from the json input file!");
	  if (get_float_from_objectlist("REC_ARRAY_DIST",number_readobjects,&(gv->REC_ARRAY_DIST),varname_list, value_list, used_list))
	    log_fatal("Variable REC_ARRAY_DIST could not be retrieved from the json input file!");
	}
      }
      if (get_float_from_objectlist("REFRECX",number_readobjects,&(gv->REFREC[1]),varname_list, value_list, used_list))
	log_fatal("Variable REFRECX could not be retrieved from the json input file!");
      if (get_float_from_objectlist("REFRECY",number_readobjects,&(gv->REFREC[2]),varname_list, value_list, used_list))
	log_fatal("Variable REFRECY could not be retrieved from the json input file!");
      if (get_float_from_objectlist("NGEOPH",number_readobjects,&(gv->NGEOPH),varname_list, value_list, used_list))
  ...
  if (get_int_from_objectlist("WRITE_MODELFILES",number_readobjects,&(gv->WRITE_MODELFILES),varname_list, value_list, used_list)) {}
  ...
  if (get_int_from_objectlist("CHECKPTREAD",number_readobjects,&(gv->CHECKPTREAD),varname_list, value_list, used_list))
    log_fatal("Variable CHECKPTREAD could not be retrieved from the json input file!");
  if (get_int_from_objectlist("CHECKPTWRITE",number_readobjects,&(gv->CHECKPTWRITE),varname_list, value_list, used_list))
  ...
  if (get_string_from_objectlist("CHECKPT_FILE",number_readobjects,(gv->CHECKPTFILE),varname_list, value_list, used_list))
  ...
  if (get_int_from_objectlist("OUT_TIMESTEP_INFO",number_readobjects,&(gv->OUTNTIMESTEPINFO),varname_list, value_list, used_list)) {}
  ...
  if (header_printed) log_warn("You may want to check %d unclaimed parameter(s).\n", not_used);
