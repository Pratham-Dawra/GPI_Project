...
float *read_srcsig(int *ns, const GlobVar *gv)
{
  ...
  *ns = 0;
  ...
  if (filetype<2) { /***** try ASCII *****/

    filep = fopen(sigfile, "r");
    if (!filep) log_fatal("Could not open file %s for reading.\n", sigfile);
    ...
    if (0==*ns && 0==gv->MPID) log_fatal("Source signature file did not contain valid samples.\n");
    
    sig = (float*)calloc(sizeof(float), *ns);
    if (!sig) log_fatal("Could not allocate source signature buffer.\n");
    ...
    while (fgets(buffer, STRING_SIZE, filep)) {  
      ...
      if (*p == '\0' || (*p == '\n') || (*p == '\r') || (*p == '#')) continue; 
      j = sscanf(p, "%e", &(sig[i]));
      if (0==j) log_warnc(0, "Scan error while reading source signature. Check result!\n");
      ++i;
    }
   
  } else if (2==filetype) { /***** try SU *****/

    filep = fopen(sigfile, "rb");
    if (!filep) log_fatal("Could not open SU file %s for reading.\n", sigfile);

    nt = su_get_nt(filep, &suns, &sudt, &sudelrt);
    if (nt>1) log_warnc(0, "SU file %s contains more than one trace. Reading only first trace.\n", sigfile);
    ...
    *ns = suns;
    ...
    if (sudelrt != 0) {
      if (0 == gv->MPID) log_fatal("Delay recording time != 0 not supported for source signature.\n");
    }

  } else {
    if (0 == gv->MPID) log_fatal("Unknown filetype in source signature read function.\n");
  }
