...
static const char *level_colors[]  = { LOG_COLOR_REDBG, LOG_COLOR_RED, LOG_COLOR_PURPLE, "", LOG_COLOR_GREEN, LOG_COLOR_BLUE };
#endif // LOG_COLOR
...
{
  if (level < LOG_WARN) log_minlevel = LOG_ERROR;
  else log_minlevel = level;
}
...
void log_general_(log_Level level, const char *file, int line, const char *fmt, ...) 
{
  ...
  switch(level) 
...
#endif
      vfprintf(fpstd, fmt, ap); fflush(fpstd);
      break;
    case LOG_STD:
      vfprintf(fpstd, fmt, ap); fflush(fpstd);
      break;
    ...
    case LOG_WARN:
      ...
      buf[strftime(buf, sizeof(buf), "%H:%M:%S", localtime(&t))] = '\0';
#ifdef LOG_COLOR
      fprintf(fpstd, "%s%-5s%s[%s,%d] ", level_colors[level], level_strings[level], LOG_ALL_RESET, 
	      buf, log_mpid_);
#else
    ...
    case LOG_ERROR:
      ...
      buf[strftime(buf, sizeof(buf), "%H:%M:%S", localtime(&t))] = '\0';
#ifdef LOG_COLOR
      fprintf(fperr, "%s%-5s%s[%s,%d] ", level_colors[level], level_strings[level], LOG_ALL_RESET,
	      buf, log_mpid_);
#else
    ...
    case LOG_FATAL:
      ...
      buf[strftime(buf, sizeof(buf), "%H:%M:%S", localtime(&t))] = '\0';
#ifdef LOG_COLOR
      fprintf(fperr, "%s%-5s[%s,%d]%s ", level_colors[level], level_strings[level], 
	      buf, log_mpid_, LOG_ALL_RESET);
#else
      fprintf(fperr, "%-5s[%s,%d] ", level_strings[level], buf, log_mpid_);
#endif
      vfprintf(fperr, fmt, ap); fflush(fperr);
...
void log_finalize() 
{
  ...
  if (rhours>0.0) sprintf(s_rhours, "%ldh ", (long)rhours);
  else s_rhours[0] = '\0';
  if (rmins>0.0 || rhours>0.0) sprintf(s_rmins, "%ldm ", (long)rmins);
  else s_rmins[0] = '\0';
  ...
  if (uhours>0.0) sprintf(s_uhours, "%ldh ", (long)uhours);
  else s_uhours[0] = '\0';
  if (umins>0.0 || uhours>0.0) sprintf(s_umins, "%ldm ", (long)umins);
  else s_umins[0] = '\0';
  ...
  if (shours>0.0) sprintf(s_shours, "%ldh ", (long)shours);
  else s_shours[0] = '\0';
  if (smins>0.0 || shours>0.0) sprintf(s_smins, "%ldm ", (long)smins);
  else s_smins[0] = '\0';
