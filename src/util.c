...
float *vector(int nl, int nh) {
  ...
  if (!v) log_fatal("Allocation failure in function vector().\n");
...
int *ivector(int nl, int nh) 
{
  ...
  if (!v) log_fatal("Allocation failure in function ivector().\n");
...
unsigned short int *usvector(int nl, int nh) 
{
  ...
  if (!v) log_fatal("Allocation failure in function usvector().\n");
  for (i=0;i<(nh-nl+1+NR_END);i++) v[i]=0;
  return v-nl+NR_END;
}
...
unsigned char *cvector(int nl, int nh) 
{
  ...
  if (!v) log_fatal("Allocation failure in function cvector().\n");
  return v-nl+NR_END;
}
...
unsigned long *lvector(int nl, int nh) 
{
  ...
  if (!v) log_fatal("Allocation failure in function lvector().\n");
  for (i=0;i<(nh-nl+1+NR_END);i++) v[i]=0;
  return v-nl+NR_END;
}
...
double *dvector(int nl, int nh) 
{
  ...
  if (!v) log_fatal("Allocation failure in function dvector().\n");
  for (i=0;i<(nh-nl+1+NR_END);i++) v[i]=0.0;
  return v-nl+NR_END;
}
...
float **fmatrix(int nrl, int nrh, int ncl, int nch) {
  ...
  if (!m) log_fatal("Allocation failure 1 in function fmatrix().\n");
  m += NR_END;
  m -= nrl;

  /* allocation rows and set pointers to them */
  m[nrl]=(float *) malloc((size_t)((nrow*ncol+NR_END)*sizeof(float)));
  if (!m[nrl]) log_fatal("Allocation failure 2 in function fmatrix().\n");
  m[nrl] += NR_END;
  m[nrl] -= ncl;
...
float **matrix(int nrl, int nrh, int ncl, int nch) 
{
  ...
  /* allocate pointers to rows */
  m=(float **) malloc((size_t) ((nrow+NR_END)*sizeof(float*)));
  if (!m) log_fatal("Allocation failure 1 in function matrix().\n");
  m += NR_END;
  m -= nrl;
  
  /* allocation rows and set pointers to them */
  m[nrl]=(float *) malloc((size_t)((nrow*ncol+NR_END)*sizeof(float)));
  if (!m[nrl]) log_fatal("Allocation failure 2 in function matrix().\n");
  m[nrl] += NR_END;
  m[nrl] -= ncl;
...
double **dmatrix(int nrl, int nrh, int ncl, int nch)
{
  ...
  if (!m[nrl]) log_fatal("Allocation failure 2 in function dmatrix().\n");
  m[nrl] += NR_END;
  m[nrl] -= ncl;
...
int **imatrix(int nrl, int nrh, int ncl, int nch)
{
  ...
  if (!m) log_fatal("Allocation failure 1 in function imatrix().\n");
  m += NR_END;
  m -= nrl;

  /* allocation rows and set pointers to them */
  m[nrl]=(int *) malloc((size_t)((nrow*ncol+NR_END)*sizeof(int)));
  if (!m[nrl]) log_fatal("Allocation failure 2 in function imatrix().\n");
  m[nrl] += NR_END;
  m[nrl] -= ncl;
...
unsigned short int **usmatrix(int nrl, int nrh, int ncl, int nch)
{
  ...
  if (!m) log_fatal("Allocation failure 1 in function usmatrix().\n");
  m += NR_END;
  m -= nrl;

  /* allocation rows and set pointers to them */
  m[nrl]=(unsigned short int *) malloc((size_t)((nrow*ncol+NR_END)*sizeof(unsigned short int)));
  if (!m[nrl]) log_fatal("Allocation failure 2 in function usmatrix().\n");
  m[nrl] += NR_END;
  m[nrl] -= ncl;
...
float ***f3tensor(int nrl, int nrh, int ncl, int nch,int ndl, int ndh)
{
  ...
  if (!t) log_fatal("Allocation failure 1 in function f3tensor().\n");
  t += NR_END;
  t -= nrl;

  /* allocate pointers to rows and set pointers to them */
  t[nrl]=(float **) malloc((size_t)((nrow*ncol+NR_END)*sizeof(float*)));
  if (!t[nrl]) log_fatal("Allocation failure 2 in function f3tensor().\n");
  t[nrl] += NR_END;
  t[nrl] -= ncl;

  /* allocate rows and set pointers to them */
  t[nrl][ncl]=(float *) malloc((size_t)((nrow*ncol*ndep+NR_END)*sizeof(float)));
  if (!t[nrl][ncl]) log_fatal("Allocation failure 3 in function f3tensor().\n");
  t[nrl][ncl] += NR_END;
  t[nrl][ncl] -= ndl;
...
int ***i3tensor(int nrl, int nrh, int ncl, int nch,int ndl, int ndh)
{
  ...
  if (!t) log_fatal("Allocation failure 1 in function i3tensor().\n");
  t += NR_END;
  t -= nrl;

  /* allocate pointers to rows and set pointers to them */
  t[nrl]=(int **) malloc((size_t)((nrow*ncol+NR_END)*sizeof(int*)));
  if (!t[nrl]) log_fatal("Allocation failure 2 in function i3tensor().\n");
  t[nrl] += NR_END;
  t[nrl] -= ncl;

  /* allocate rows and set pointers to them */
  t[nrl][ncl]=(int *) malloc((size_t)((nrow*ncol*ndep+NR_END)*sizeof(int)));
  if (!t[nrl][ncl]) log_fatal("Allocation failure 3 in function i3tensor().\n");
  t[nrl][ncl] += NR_END;
  t[nrl][ncl] -= ndl;
...
float ****f4tensor(int nrl, int nrh, int ncl, int nch,int ndl, int ndh, int nvl, int nvh)
{
  ...
  /* allocate pointers to pointers to rows */
  t=(float ****) malloc((size_t) ((nrow+NR_END)*sizeof(float**)));
  if (!t) log_fatal("Allocation failure 1 in function f4tensor().\n");
  t += NR_END;
  t -= nrl;

  /* allocate pointers to rows and set pointers to them */
  t[nrl]=(float ***) malloc((size_t)((nrow*ncol+NR_END)*sizeof(float*)));
  if (!t[nrl]) log_fatal("Allocation failure 2 in function f4tensor().\n");
  t[nrl] += NR_END;
  t[nrl] -= ncl;

  /* allocate rows and set pointers to them */
  t[nrl][ncl]=(float **) malloc((size_t)((nrow*ncol*ndep+NR_END)*sizeof(float)));
  if (!t[nrl][ncl]) log_fatal("Allocation failure 3 in function f4tensor().\n");
  t[nrl][ncl] += NR_END;
  t[nrl][ncl] -= ndl;

  /* allocate values and set pointers to them */
  t[nrl][ncl][ndl]=(float *) malloc((size_t)((nrow*ncol*ndep*nval+NR_END)*sizeof(float)));
  if (!t[nrl][ncl][ndl]) log_fatal("Allocation failure 4 in function f4tensor().\n");
