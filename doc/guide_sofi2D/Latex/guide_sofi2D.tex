...
\path{make all} or \path{make sofi2D}

to compile the program. You may also use the shell script \path{compileSOFI2D.sh} located in \path{sofi2D/par}. For a successful compiler run you probably need to change the compiler options in \path{sofi2D/src/Makefile}. 

To run the program on 8 CPUs do

\path{cd sofi2D/par}

\path{mpirun -np 8 ../bin/sofi2D ./in_and_out/sofi2D.json}


You may also use the shell script \path{startSOFI2D.sh} located in \path{sofi2D/par} that also writes the screen output to the file \path{sofi2D/par/in_and_out/sofi2D.jout}.

The modeling parameters are specified in the sofi2D input file \path{./in_and_out/sofi2D.json}. The parameters should be more or less self-explanatory. The synthetic seismograms and snapshots are written to the files specified in the sofi2D input file.
...
SOFI2D is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more details. You should have received a copy of the GNU General Public License along with SOFI2D. See file COPYING and/or \path{http://www.gnu.org/licenses/gpl-2.0.html}.

The Authors of SOFI2D are listed in file \path{AUTHORS}.
...
\EQ{grid_disp:2}{\frac{d v_x}{d x}\biggl|_{x=0}=\frac{2 \pi}{\lambda} \cos\biggl(2 \pi \frac{x}{\lambda}\biggr)\biggl|_{x=0}=2 \pi.}

In the next step, the derivation is approximated numerically by a 2nd order finite-difference operator:

\EQ{grid_disp:3}{\frac{d v_x}{d x}\biggl|_{x=0} \approx \frac{v_x(x+\Delta x)-v_x(x)}{\Delta x}\biggl|_{x=0}=\frac{\sin \biggl(\frac{2 \pi \Delta
...
\begin{figure}[ht]
\begin{center}
\epsfig{file=eps/homogenous_grid_n_16_5.eps, width=7 cm}\epsfig{file=eps/homogenous_grid_n_16_10.eps, width=7 cm}
\epsfig{file=eps/homogenous_grid_n_4_5.eps, width=7 cm}\epsfig{file=eps/homogenous_grid_n_4_10.eps, width=7 cm}
\epsfig{file=eps/homogenous_grid_n_2_5.eps, width=7 cm}\epsfig{file=eps/homogenous_grid_n_2_10.eps, width=7 cm}
...
\begin{figure}[ht]
\begin{center}
\epsfig{file=eps/courandt_1.eps, width=7 cm}\epsfig{file=eps/courandt_2.eps, width=7 cm}
\epsfig{file=eps/courandt_3.eps, width=7 cm}\epsfig{file=eps/courandt_4.eps, width=7 cm}
...
i.e. you must log on node 0 (first line in the file par/lamhosts) and run the software on this node. In the last stable release of LAM-MPI, the node 0 just has to be listed in the lamhosts file (the order does not matter). To boot lam just do \path{lamboot -v par/lamhosts}. To run the software in serial on a single PC just type \path{lamboot} without specifying a lamhosts file. You can still specify more PEs in the FD software but all processes will be executed on the same CPU. Thus this only makes sense if you run the software on a multicore system. In this case, you should boot it without
a lamhosts file and specify a total number of 2 processing elements (PEs). To shut down LAM again (before logout) use the command \path{lamclean -v}. However, it is not necessary to shut down/restart LAM after each logout/login. 

\section{Installation}
\label{installation}
After unpacking the software package (e.g. by \path{tar -zxvf sofi2D.tgz}) and changing to the directory sofi2D (\path{cd sofi2D})  you will find different subdirectories:
...
\textbf{bin}\\
This directory contains all executable programs, generally sofi2D and snapmerge. These executables are generated using the command \path{make $<$program$>$} (see below).
...
\label{modelgeom}
The geometry of the FD grid and all parameters for the wave field simulation have to be defined in a parameter file (which we name in this case {\path{sofi2D/par/in_and_out/sofi2D.json}}). In the following we will first list the full input file and later explain every input parameter section in detail:
\input{sofi2D.json}
...
In our example, we specify 100 receiver location. Due to the small size of the model, most of the specified receiver positions will be located inside this absorbing boundary (see Chapter \ref{abs}). A corresponding warning message will appear. If you choose to read the receiver location from REC\_FILE \path{./receiver/receiver.dat} (READREC=1), only 10 receivers locations are considered. The list of receivers specified in file \path{./receiver/receiver.dat} is equivalent to the parameters in the input file with only a larger distance between adjacent receivers (NGEOPH = 10.)
...
SEIS\_FILE : basic filename according to output of seismograms (SEISMO), separate seismogram files are outputted and will look like \path{SEIS_FILE_vz.su} or \path{SEIS_FILE_div.bin.0}\\
...
If you choose to create the model ``on the fly'', the distribution of the material parameters P-wave velocity $v_p$, S-wave velocity $v-s$ and density $\rho$ for the simple block model has to be defined in the function \path{hh_elastic.c} that can be found in the \path{src} folder.
...
To compile the program sofi2D you must change to the \path{src} directory and execute:
...
Each sofi2D run reads the required parameters from a parameter file. An example parameter file was defined in section \ref{modelgeom} \path{par/in_and_out/sofi2D.json}. The command to start a simulation on 4 processor with the lowest priority of -19 (in order to allow working on the a workstation while running a simulation) is as follows. Please note, that we assume you have navigated to the folder sofi2D/par.

\path{mpirun -np 8 nice -19 ../bin/sofi2D ./in_and_out/sofi2D.json }

If you use LAMMPI the command \path{lamboot -v lamhost} must be executed on node 0 which is the PE where ./par/lamhosts is the file containing IP addresses of all computing nodes. It if often useful to save the standard output of the program for later reference. The screen output may be saved to \path{sofi2D.jout} using 

\path{mpirun -np 8 nice -19 ../bin/sofi2D ./in_and_out/sofi2D.json > ./in_and_out/sofi2D.jout}

Combining both the screen and the file output you can execute the following line (this is used in the shell script \path{startSOFI2D.sh})

\path{mpirun -np 8 nice -19 ../bin/sofi2D ./in_and_out/sofi2D.json | tee ./in_and_out/sofi2D.jout}
...
The wavefield snapshots can be merged using the program \path{snapmerge}. The program snapmerge is not a MPI program. Therefore, it can be executed without MPI and the mpirun command. You can run snapmerge on any PC since a MPI environment (e.g. LAM) is not required. You may therefore copy the snapshot outputs of the different nodes to another non-MPI computer to merge the files together. \path{snapmerge} reads the required information from the sofi2D parameter file. Simply type
