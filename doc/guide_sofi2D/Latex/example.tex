...
\texttt{WEQ} : Wave equation type (acoustic=1; viscoacoustic=2; elastic=3; viscoelastic=4; elastic VTI=5; viscoelastic VTI=6; elastic TTI=7; viscoelastic TTI=8)
...
\texttt{SIGOUT} : Output source wavelet (yes=1; no=else)\\
...
SU-files can be merged together using the Unix command \texttt{cat}. For example, in order to merge seismograms of the $vx$-component of particle velocity into one single SU-file use: \texttt{cat SEIS\_FILE\_VR.* $>$ SEIS\_FILE\_VR}. The shell script \textt{sucat.sh} merges all seismograms at the same time.\\
